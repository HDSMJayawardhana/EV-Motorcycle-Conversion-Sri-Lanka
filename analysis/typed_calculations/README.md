# 💻 Typed Engineering Calculations

Formatted versions of all engineering calculations for the 
EV motorcycle conversion project.

## 📄 Files

| File | Description |
|------|-------------|
| `forces_cal.pdf` | Updated technical calculations for 1000W BLDC at 40 km/h |
| `power_range_weight.pdf` | Power, range estimation, and weight balancing |
| `motor_battery_selection.pdf` | Complete motor and battery selection summary |
| `Cost-Benefit.pdf` |  Monthly operating cost, Monthly saving, Payback period |
